...
import { CheckCircle, Download, Home, Ship } from 'lucide-react'
import { useEffect, useState } from 'react'

export default function PaymentSuccess() {
  const [sp]     = useSearchParams()
  const navigate = useNavigate()
  const ref   = sp.get('ref')   ?? 'LCXXXXXX'
  ...
  return (
    <div style={{
      minHeight: '100vh', background: 'linear-gradient(160deg, #0a1628 0%, var(--navy) 60%, #0d2340 100%)',
        ...
        <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 0 }}>
          {[...Array(24)].map((_, i) => (
            <div key={i} style={{
              ...
              background: ['var(--gold)', '#fff', '#C8A951', '#E2C97A'][i % 4],
              animation: `fall ${2 + Math.random() * 2}s linear ${Math.random() * 2}s forwards`,
            }} />
          ))}
          <style>{`@keyframes fall { to { top: 110%; opacity: 0; transform: rotate(360deg); } }`}</style>
        </div>
      )}

      <div style={{ position: 'relative', zIndex: 1, maxWidth: 520, width: '100%', textAlign: 'center' }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ width: 96, height: 96, borderRadius: '50%', background: 'rgba(200,169,81,0.15)',
        ...
        <div style={{ background: 'rgba(255,255,255,0.07)', backdropFilter: 'blur(20px)',
                       border: '1px solid rgba(255,255,255,0.12)', borderRadius: 24, padding: '36px 32px' }}>
          <p style={{ color: 'var(--gold)', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.15em',
                       textTransform: 'uppercase', marginBottom: 8 }}>Voyage Confirmed</p>
          <h1 style={{ fontFamily: "'Playfair Display', serif", color: 'white',
                        fontSize: 'clamp(1.5rem,3vw,2rem)', margin: '0 0 6px' }}>
            Thank You for Choosing
          </h1>
          <h1 className="text-gold-gradient" style={{ fontFamily: "'Playfair Display', serif", margin: '0 0 28px' }}>
            LUXE Cruises
          </h1>

          <p style={{ color: 'rgba(255,255,255,0.7)', marginBottom: 28, lineHeight: 1.6, fontSize: '0.9rem' }}>
            Your voyage is confirmed! A detailed itinerary and boarding pass will be sent to your email.
          </p>
          ...
          <div style={{ background: 'rgba(200,169,81,0.12)', border: '1.5px solid rgba(200,169,81,0.4)',
            ...
            <p style={{ margin: '0 0 4px', color: 'rgba(255,255,255,0.6)', fontSize: '0.75rem',
                         textTransform: 'uppercase', letterSpacing: '0.1em' }}>Booking Reference</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                         padding: '14px 20px', background: 'rgba(255,255,255,0.05)', borderRadius: 12, marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button onClick={() => navigate('/')}
                    style={{ flex: 1, padding: '12px 20px', background: 'rgba(255,255,255,0.08)',
                              border: '1.5px solid rgba(255,255,255,0.2)', borderRadius: 12,
                              fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer', color: 'white',
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
          <Ship size={14} color="rgba(255,255,255,0.3)" />
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.78rem', margin: 0 }}>
            Check-in opens 48 hours before departure · Arrive at port 3 hours early
          </p>
        </div>
      </div>
    </div>
