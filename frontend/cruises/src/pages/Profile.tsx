...
import { cruiseBookingsApi, authApi, CruiseBooking } from '../api'
...
export default function Profile() {
  ...
  const [bookings, setBookings] = useState<CruiseBooking[]>([])
  ...
  const statusColor = (s: string) => {
    if (s === 'confirmed' || s === 'completed') return { bg: '#f0fdf4', color: '#16a34a' }
    if (s === 'cancelled') return { bg: '#fef2f2', color: '#ef4444' }
    return { bg: '#fffbeb', color: '#d97706' }
  }
  const statusIcon = (s: string) => {
    if (s === 'confirmed' || s === 'completed') return <CheckCircle size={13} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f9fb', padding: '32px 16px' }}>
      <div className="max-w-5xl mx-auto" style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            {([
              ...
              { id: 'bookings', label: 'My Voyages',  Icon: BookOpen },
            ...
            <div style={{ borderTop: '1px solid #f3f4f6' }}>
              <button onClick={() => { clearAuth(); navigate('/') }}
                      style={{ width: '100%', padding: '13px 18px', border: 'none', cursor: 'pointer',
                                textAlign: 'left', fontSize: '0.875rem', background: 'white',
        ...
        {/* Main panel */}
        <div style={{ background: 'white', borderRadius: 16, padding: '28px', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>

          {/* Profile tab */}
          {activeTab === 'profile' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h2 style={{ margin: 0, fontWeight: 700, color: 'var(--navy)', fontSize: '1.1rem' }}>My Profile</h2>
                {!editMode ? (
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => setEditMode(false)}
                            style={{ padding: '7px 16px', border: '1.5px solid #e5e7eb', borderRadius: 8, background: 'none', cursor: 'pointer', fontSize: '0.82rem' }}>
                      Cancel
                    </button>
                    <button onClick={handleSave} disabled={saving}
                            style={{ padding: '7px 16px', border: 'none', borderRadius: 8, background: 'var(--gold)', color: 'var(--navy)', fontWeight: 700, cursor: 'pointer', fontSize: '0.82rem' }}>
                      {saving ? 'Saving…' : 'Save'}
                    </button>
                  </div>
                )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ...
                  <div key={key} style={{ gridColumn: span ? '1 / -1' : undefined }}>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#6b7280',
                    ...
                    <input
                      ...
                      style={{ width: '100%', padding: '10px 14px', border: '1.5px solid #e5e7eb',
                                borderRadius: 10, fontSize: '0.9rem', color: 'var(--navy)', outline: 'none',
                                background: (!editMode || disabled) ? '#f9fafb' : 'white', boxSizing: 'border-box' as const }}
                    />
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'bookings' && (
            <div>
              <h2 style={{ margin: '0 0 24px', fontWeight: 700, color: 'var(--navy)', fontSize: '1.1rem' }}>
                My Voyages
              </h2>
              {loadingBookings ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 0' }}>
                  ...
                  <p style={{ fontWeight: 600, color: 'var(--navy)', marginBottom: 6 }}>No voyages yet</p>
                  <p style={{ color: '#9ca3af', fontSize: '0.875rem' }}>Your cruise bookings will appear here</p>
                      ...
                      <div key={b.id} style={{ padding: '16px 18px', border: '1.5px solid #f0f0f0', borderRadius: 14,
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                            <span style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--navy)' }}>{b.cruise_name}</span>
                          ...
                          <p style={{ margin: 0, fontSize: '0.8rem', color: '#6b7280' }}>
                            ...
                            {' · '}{b.passenger_name}{' · '}{b.num_guests} guest{b.num_guests > 1 ? 's' : ''}
                          </p>
                        </div>
          ...
          {activeTab === 'support' && (
            <div>
              <h2 style={{ margin: '0 0 24px', fontWeight: 700, color: 'var(--navy)', fontSize: '1.1rem' }}>Support</h2>
              {[
                { icon: '📞', title: 'Call Us',       sub: '1800-LUXE-SEA (24/7 Toll-Free)' },
                { icon: '✉️', title: 'Email Support', sub: 'support@luxecruises.com · Reply in 2h' },
                { icon: '💬', title: 'Live Chat',     sub: 'Available Mon–Sat 6AM–10PM IST' },
                { icon: '📋', title: 'FAQs',          sub: 'Boarding, luggage, shore excursions & more' },
              ].map(({ icon, title, sub }) => (
